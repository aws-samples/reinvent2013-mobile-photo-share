...
public class TemporaryCredentialManagement {
    ...
    protected String getPolicyObject(String username) {
        // Ensure the username is valid to prevent injection attacks.
        if (!Utilities.isValidUsername(username)) {
            throw new IllegalArgumentException("Invalid Username");
        }
        return Utilities.getRawPolicyFile( "/IdentityTokenVendingMachinePolicy.json" ).replaceAll("__USERNAME__", username)
                .replaceAll("__REGION__", Configuration.DYNAMODB_REGION)
                .replaceAll("__ACCOUNT_ID__", Configuration.AWS_ACCOUNT_ID)
                .replaceAll("__USERS_TABLE__", Configuration.USERS_TABLE)
