...
#import "AmazonIdentityTVMClient.h"
...
#import "GetIdentityTokenRequest.h"
...
@implementation AmazonIdentityTVMClient
...
-(Response *)getToken
{
    NSString         *uid = [AmazonTVMKeyChainWrapper getUidForDevice];
    NSString         *key = [AmazonTVMKeyChainWrapper getKeyForDevice];

    Request          *request = [[GetIdentityTokenRequest alloc] initWithEndpoint:self.endpoint andUid:uid andKey:key usingSSL:self.useSSL];
    ...
    if ( [response wasSuccessful]) {
        [AmazonTVMKeyChainWrapper storeCredentialsInKeyChain:response.accessKey secretKey:response.secretKey securityToken:response.securityToken expiration:response.expirationDate];
    }
    else {
...
-(Response *)login:(NSString *)username password:(NSString *)password
{
    ...
    if ( [AmazonTVMKeyChainWrapper getUidForDevice] == nil) {
        NSString        *uid = [Crypto generateRandomString];

        LoginRequest    *request = [[LoginRequest alloc] initWithEndpoint:self.endpoint andUid:uid andUsername:username andPassword:password andAppName:self.appName usingSSL:self.useSSL];
        ...
        if ( [response wasSuccessful]) {
            [AmazonTVMKeyChainWrapper registerDeviceId:uid andKey:((LoginResponse *)response).key];
            [AmazonTVMKeyChainWrapper storeUsername:username];
        }
        else {
            AMZLogDebug(@"Token Vending Machine responded with Code: [%d] and Messgae: [%@]", response.code, response.message);
        }
...
{
    int             retries   = 2;
    RequestDelegate *delegate = [RequestDelegate new];
